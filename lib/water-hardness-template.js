...
function productUrl(band) {
  ...
  return param
    ? `https://agsoftener.com/?band=${encodeURIComponent(param)}`
    ...
    <section class="reveal-section">
      ...
      <div class="panel ag-card">
        ...
        <div>
          ...
          <a class="button-link" href="${productUrl(system.band)}" target="_blank" rel="noopener">SEE HOW AG SOFTENER WORKS</a>
        </div>
      </div>
    </section>
  `;
...
function renderIndex(systems) {
  ...
  const html = pageShell({
    ...
    metaDesc: "Verified water hardness numbers for " + systems.length + " US cities, sourced from utility reports. See the hardest cities ranked and check your own zip.",
    headExtra,
    bodyContent
  });
